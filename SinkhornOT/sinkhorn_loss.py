...
def sinkhorn_iteration(C, mu, nu, epsilon, numIterMax=100, tol=1e-9, debug=True):
    """
...
def forward_relax_sinkhorn_iteration(C, mu, nu, lambdda, epsilon, numIterMax=100, tol=1e-6, debug=False):
    ...
    *_, J1 = nu.shape
    ...
    u = torch.zeros_like(mu).double()  # Kantorovich potential for source distribution

    b = torch.ones_like(nu).double()  # partial update of Kantorovich potential
    v = torch.zeros_like(nu).double()  # Kantorovich potential for target distribution
    K = K_calc(u, v)
    transport = torch.sum(torch.sum(torch.mul(K, C), -1), -1).squeeze()
    for ii in range(numIterMax):
        s = torch.sum(torch.mul(K, b), -1, keepdim=True)
...
if __name__ == "__main__":
    ...
    a_ten, b_ten = torch.from_numpy(a).view(1, n, 1), torch.from_numpy(b).view(1, 1, m)
    C_ten = torch.from_numpy(C).view(1, n, m)
    T_ten = sinkhorn_iteration(C_ten, a_ten, b_ten, epsilon=0.01)

    err = torch.norm(T_ten[0] - torch.from_numpy(Tarr))
